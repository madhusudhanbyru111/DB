...
file = pd.read_csv('InputTable.csv')
...
def bcnfDecomposition(rel, dependencies):
    decomposed_t = []

    for determinent, dependent in dependencies.items():
        closure_set = closureMethod(set(determinent), dependencies)
...
def check2NF(primary_key, dependencies, rel):
...
def check3NF(rels, dependencies):
...
def checkBCNF(rels, primary_key, dependencies):
    for rel_name, rel in rels.items():
        all_attri = set(rel.columns)
        for determinent, dependents in dependencies.items():
            for dependent in dependents:
                if dependent not in determinent:
                    if all_attri - closureMethod(determinent, dependencies):
                        return False
    return True


def check4NF(rels, multivalue):
    for rel_name, rel in rels.items():
...
#Normalizer:
def normalizeTo1NF(rel, primary_key):
    rels = {}
    one_flag = check1NF(rel)
    ...
    if one_flag:
        ...
        for col in rel.columns:
            if rel[col].apply(isListOrSet).any():
                rel = rel.explode(col)
        rels[primary_key] = rel
        return rels, one_flag

def normalizeTo2NF(rel, primary_key, dependentencies):
    rel = rel[primary_key]
    rels = {}
    rm_cols = []
    two_flag = check2NF(primary_key, dependentencies, rel)
...
def normalizeTo3NF(rels, primary_key, dependencies):
    three_rels = {}
    original_rel = rels
    three_flag = check3NF(rels, dependencies)
...
def normalizeToBCNF(rels, primary_key, dependencies):
    bcnf_rels = {}
    bcnf_final = {}
    bcnf_flag = checkBCNF(rels, primary_key, dependencies)
...
def normalizeTo4NF(rels, multivalue):
    four_rels = {}
    four_flag = check4NF(rels, multivalue)
    ...
    if len(four_rels) == len(rels):
        ...
        return normalizeTo4NF(four_rels, multivalue)


def checkLosslessFor5NF(rel_name, dataframe, candidate_keys):
...
def normalizeTo5NF(rels, primary_key, dependencies):
    five_rels = {}
    five_flag, candidate_keys_dict = check5NF(rels)
    ...
    if five_flag:
        ...
        for rel_name, rel in rels:
            candidate_keys = candidate_keys_dict[rel_name]
            decomposed_rels = checkLosslessFor5NF(
...
def get1NF(primary_keys, df):
    ...
    for col, dtype in zip(df.columns, df.dtypes):
        if col in primary_keys:
            query += f"  {col} {outputTypeMethod(dtype)} PRIMARY KEY,\n"
        else:
            query += f"  {col} {outputTypeMethod(dtype)},\n"
...
if max_normal_form == 'B' or max_normal_form >= 1:
    onenf_table, one_flag = normalizeTo1NF(
        file, primary_key)
    if one_flag:
    ...
    if max_normal_form == 1:
        ...
        get1NF(primary_key, onenf_table)

if max_normal_form == 'B' or max_normal_form >= 2:
    twonf_tables, two_flag = normalizeTo2NF(
...
if max_normal_form == 'B' or max_normal_form >= 3:
    threenf_tables, three_flag = normalizeTo3NF(
...
if max_normal_form == 'B' or max_normal_form >= 4:
    bcnf_tables, bcnf_flag = normalizeToBCNF(
...
if not max_normal_form == 'B' and max_normal_form >= 4:
    fournf_tables, four_flag = normalizeTo4NF(
...
if not max_normal_form == 'B' and max_normal_form >= 5:
    fivenf_tables, five_flag = normalizeTo5NF(
